...
class TrackWarrantControllerIntegrationTest {
    ...
    void testCreateWarrant() throws Exception {
        CreateWarrantRequest request = CreateWarrantRequest.builder()
            ...
            .line2Instruction("Proceed from Station A to Station B on Test Line Track")
            .build();
        ...
        mockMvc.perform(post("/api/warrants")
            ...
            .andExpect(jsonPath("$.line2Instruction").value("Proceed from Station A to Station B on Test Line Track"))
    ...
    void testCompleteWorkflow() throws Exception {
        ...
        CreateWarrantRequest request = CreateWarrantRequest.builder()
            ...
            .line2Instruction("Proceed from Start Point to End Point on Main Track")
            .build();
    ...
    void testCreateWarrantRoundTripAllEntryFormFields() throws Exception {
        ...
        CreateWarrantRequest request = CreateWarrantRequest.builder()
            ...
            .line1Instruction("Track Warrant NO. TW-FULL-001 is Void.")
        ...
        mockMvc.perform(post("/api/warrants")
            ...
            .andExpect(jsonPath("$.line1Instruction").value("Track Warrant NO. TW-FULL-001 is Void."))
            .andExpect(jsonPath("$.line2Instruction").value("Proceed from RIVER JCT to NORTH YARD on MAIN Track"))
            .andExpect(jsonPath("$.line3Instruction").value("Proceed from RIVER JCT to NORTH YARD on MAIN Track"))
            .andExpect(jsonPath("$.line4Instruction").value("Work between SIDING A and SIDING B on SIDING Track"))
            .andExpect(jsonPath("$.line5Instruction").value("Not in effect until 09:45M"))
            .andExpect(jsonPath("$.line6Instruction").value("This Authority Expires at 17:30M"))
            .andExpect(jsonPath("$.line7Instruction").value("Not in effect until after arrival of TRAIN 12 at WEST JCT"))
            .andExpect(jsonPath("$.line8Instruction").value("Hold Main Track at Last Named Point."))
            .andExpect(jsonPath("$.line9Instruction").value("Do Not Foul Limints ahead of WEST JCT."))
            .andExpect(jsonPath("$.line10Instruction").value("Clear Main Track at Last Named Point."))
            .andExpect(jsonPath("$.line11Instruction").value("Between MP 10 and MP 12 Make All Movements at restricted speed. Limits occupied by train or engine."))
            .andExpect(jsonPath("$.line12Instruction").value("Between MP 9 and MP 13 Make All Movements at restricted speed and stop short of Men or Machines fouling track."))
            .andExpect(jsonPath("$.line13Instruction").value("Do not exceed 45 MPH between RIVER JCT and NORTH YARD"))
            .andExpect(jsonPath("$.line14Instruction").value("Do not exceed 45 MPH between SIDING A and SIDING B"))
            .andExpect(jsonPath("$.line15Instruction").value("Protection as prescribed by Rule 99 not required."))
            .andExpect(jsonPath("$.line16Instruction").value("Track Bulletins in Effect: TB-01, TB-02, TB-03"))
            .andExpect(jsonPath("$.line17Instruction").value("Other Specific Instructions: Reduce speed through work zone"))
            .andExpect(jsonPath("$.status").value("ACTIVE"))
            .andExpect(jsonPath("$.createdDateTime").exists())
            .andExpect(jsonPath("$.lastModifiedDateTime").exists());
    }

    // Helper method to create test warrants
    private void createTestWarrant(String warrantId, String trainId, TrackWarrant.WarrantStatus status) {
        TrackWarrant warrant = TrackWarrant.builder()
            ...
            .copiedBy("TEST CREW")
            .line2Instruction("Proceed from Test Start to Test End on Test Track Track")
            .build();
